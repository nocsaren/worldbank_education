---
title: "Example: Linear Model with Worldbank Data"
...
To begin our analysis, we obtain data from the Worldbank API. We have chosen to investigate two key indicators: PISA Reading Scores and Education Expenditure (% of GDP). To efficiently retrieve data from the API, we have created a custom R function, `get_specific_id`. This function constructs the appropriate URL and fetches data from the Worldbank API. It also saves the data as RDS files for later use. Additionally, we provide a function, `search_id`, to search for specific Worldbank indicators based on keywords.
...
## Preparing Data
After obtaining the data, we import it into R as dataframes, creating two essential datasets: PISA Reading Scores and Education Expenditure. These datasets are saved as RDS files to ensure easy access for future analysis. We also perform some initial data cleaning and join these datasets based on common country-date pairs, creating a comprehensive dataset for our analysis. The data preparation phase ensures that we work with high-quality data for our regression analysis.
...
```
The joined dataframe looks like this:
``` {r joined, echo=FALSE}
head(joined)
```
## Analysis

In this section, we delve into the heart of our investigation, exploring the relationship between two variables—PISA Reading Scores and Education Expenditure. We aim to determine whether there exists a statistically significant association between these two factors.
...
Before diving into the analysis, we showed how to efficiently retrieve data from the Worldbank API. We introduced a custom R function, `get_specific_id`, which allows us to fetch relevant data, saving it as RDS files for easy access in the future. Additionally, we provided the `search_id` function to search for specific Worldbank indicators based on keywords.

Next, we imported the obtained data into R as dataframes, creating two essential datasets: PISA Reading Scores and Education Expenditure. These datasets were saved as RDS files for easy access in subsequent analysis. We conducted initial data cleaning and merged these datasets based on common country-date pairs, creating a comprehensive dataset for our analysis.
